...
我司指标平台涉及公式解析，但是这部分都放在后端做了，客户端只需要对主站返回的数据展示即可；不料后台对指标公式中的某些DRAW绘制相关的说明无法转成数据源，所以最后折中后客户端需要对绘制的语句进行解析出字段ID和要绘制的图形。

命令如下：

```c
DRAWBAND(VAL1, RBG(0, 224, 224),VAL2, RBG(0,255,255) ), LineStyle;
DRAWTEXT(COND, PRICE ,TEXT),coloryan,LineStyle;
```

其实吧现在有三种选择：一、写个Mini解释器；二、傻瓜式遍历；三、正则匹配

1. 第一种方案有点杀鸡用牛刀的赶脚，毕竟要解释的就一些绘制相关的语句，花大力气、时间，得到的收益却很小，所以这个方案暂且搁置，除非之后有更多的需要解析；
2. 第二种就是遍历字符串，我们必定有保留关键字，正如上面的"DRAWBAND"和"DRAWTEXT"，如果第一个字符匹配，那就继续找下去直到完全匹配整个单词，接着匹配左括号"("，知道要读入第一个参数了，这里有个问题，我们可能需要根据“DRAWBAND” 和“DRAWTEXT” 做分支处理，毕竟读入的参数不同，才这么点就已经让人头疼不已了。。。
3. 第三种就简单很多了，先匹配"DRAWBAND(" 打头，")"结尾的语句，pattern:`DRAWBAND\\(.*\\)`，Note：正则匹配是贪婪算法，所以总是匹配最长的，最右侧的括号会尽量往后匹！若想最短匹配，则用"?"，pattern:`DRAWBAND\\(.*?\\)`。

更多正则匹配：[点击这里](https://deerchao.net/tutorials/regex/regex.htm)